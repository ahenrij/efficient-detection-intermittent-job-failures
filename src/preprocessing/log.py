...
def clean(log: str):
    ...
    log = re.sub(r"([A-Za-z]+[\d@]+[\w@]*|[\d@]+[A-Za-z]+[\w@]*)", "<ID>", log) # remove remaining IDs
    log = re.sub(r"\s[a-zA-Z]$", "\n", log, flags=re.M) # remove remaining single characters
    log = re.sub(r"(?<!(exit|status) code:? )\d", "<NUMBER>", log) # remove single numbers that are not a status code
