...
def compute_metrics(y_pred, y_test) -> Dict[str, float]:
    ...
    """
    report = metrics.classification_report(y_true=y_test, y_pred=y_pred, output_dict=True, zero_division=0)
...
def f1_score(y_pred, y_test) -> float:
    ...
    return metrics.f1_score(y_true=y_test, y_pred=y_pred, average="binary")
